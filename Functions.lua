...
function MOVE(frame)
  ...
  if frame.origonupdate then
    ...
    frame:SetScript('OnUpdate', function(self, ...)
      if self.origonupdate then
        self.origonupdate(self, ...)
      end
...
function AnimRotOnUpdate(self, step)
  if not self:GetParent():GetParent().angle then
    self:GetParent():GetParent().angle = 0
  end
  self.step = step
  RotateTex(
    self:GetParent():GetParent(),
    self:GetParent():GetParent().angle + self:GetSmoothProgress() * self.step
  )
end

function AnimRotOnUpdate2(self)
  local tex = self.tex
  if not tex.angle then
    tex.angle = 0
  end
