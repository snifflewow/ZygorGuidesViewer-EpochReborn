...
function MapSpot:New(spottype, level)
  return setmetatable(
    { ['type'] = spottype or 'at', ['level'] = level, ['objects'] = {} },
    ZGV.MapSpotProto_mt
  )
end
    ...
    for o, obj in ipairs(self.objects) do
      if
        ...
        and (
          ...
          or (
            objtype == 'drop'
            and not obj.item.ore
            and not obj.item.skin
            and not obj.item.herb
            and not obj.item.vendor
            and not obj.vendor
          )
        )
      then
...
local function MapSpot_ObjectToString(obj)
  if obj.type == 'item' and not obj.hidden then
    ...
    local perc = ''
    if obj.perc then
      perc = (' (%d%%)'):format(obj.perc)
    end
    if obj.toohard then
      local skill = ''
      if obj.item.ore then
        skill = (' (Mining %d)'):format(obj.item.ore)
      end
      if obj.item.skin then
        skill = (' (Skinning %d)'):format(obj.item.skin)
      end
      if obj.item.herb then
        skill = (' (Herbalism %d)'):format(obj.item.herb)
      end
...
function MapSpot:UpdateVisibility()
  ...
  if self.waypoint then
    self.waypoint.hidden = self.hidden
  end
end
...
function MapSpot:FillTooltip(tooltip)
  ZGV:Debug('Filling tooltip (MS set)')
  MS = self
  if self.hidden then
    return false
  end
    ...
    for s, source in ipairs(itemsources) do
      ...
      if objs then
        ...
        for o, obj in ipairs(objs) do
          if obj.toohard then
            tooltip:AddLine(obj.iconstring, 1, 0, 0, true)
          else
            tooltip:AddLine(obj.iconstring, 1, 1, 1, true)
          end
        end
      end
    end
