...
local _G, assert, table, string, tinsert, tonumber, tostring, type, ipairs, pairs, setmetatable, math, wipe =
  _G,
  assert,
  table,
  string,
...
function Pointer:SetWaypoint(c, z, x, y, data)
  ...
  if not data.icon then
    data.icon = ZGV.DIR .. '\\Skin\\minimaparrow-green-dot'
  end
  if not data.edgeicon then
    data.edgeicon = ZGV.DIR .. '\\Skin\\minimaparrow-green-edge'
  end
...
function Pointer:RemoveWaypoint(waypoint)
  ...
  if self.ArrowFrame.waypoint == waypoint then
    self:HideArrow()
  end
...
function Pointer:ShowArrow(waypoint)
  ...
  Astrolabe:PlaceIconOnMinimap(
    waypoint.minimapFrame,
    waypoint.c,
    waypoint.z,
    waypoint.x,
    waypoint.y
  ) -- if it's not already there, place it
...
function Pointer:GetMarker()
  ...
  marker.minimapFrame = CreateFrame(
    'Button',
    'ZGVMarker' .. nummarkers .. 'Mini',
    Minimap,
    'ZygorGuidesViewerPointerMinimapMarker'
  )
  marker.worldmapFrame = CreateFrame(
    'Button',
    'ZGVMarker' .. nummarkers .. 'World',
    self.OverlayFrame,
    'ZygorGuidesViewerPointerWorldMapMarker'
  )
...
function markerproto:UpdateWorldMapIcon(c, z)
  local show = true
  if not ZGV.Pointer.OverlayFrame:IsShown() or self.hidden then
    show = false
  end
  ...
  if show and not self.overworld then
    ...
    if self.c ~= c or self.z ~= z then
      show = false
    end
  end

  if show then
    local x, y = Astrolabe:PlaceIconOnWorldMap(
      ZGV.Pointer.OverlayFrame,
      self.worldmapFrame,
      self.c,
      self.z,
      self.x,
...
function markerproto:UpdateMiniMapIcon(c, z)
  ...
  if
    ...
    and not self.hidden
    and (
      self.onminimap == 'always'
      or ZGV.Pointer.ArrowFrame.waypoint == self
      or (
        (self.onminimap == 'zone' or self.onminimap == 'zonedistance')
        and c == self.c
        and z == self.z
      )
    )
...
function Pointer:CreateArrowFrame()
  self.ArrowFrame = CreateFrame(
    'Frame',
    'ZygorGuidesViewerPointerArrowFrame',
    UIParent,
    'ZygorGuidesViewerFloatingArrow'
  )
...
local function ShowTooltip(button, tooltip)
  if not button.waypoint or not button.waypoint.t then
    return
  end
...
function Pointer.MinimapButton_OnUpdate(self, elapsed)
  ...
  self.dist = dist
  if self.waypoint.OnUpdate then
    self.waypoint:OnUpdate()
  end
      ...
      if minimapcontrol_suspension > 0 then
        ...
        if dist ~= lastminimapdist then
          ...
          if old_minimap_lastset ~= minimap_lastset then
            -- sanitise buttons
            if minimap_lastset == (Minimap:GetZoomLevels() - 1) then
              MinimapZoomIn:Disable()
            else
              MinimapZoomIn:Enable()
            end
            if minimap_lastset == 0 then
              MinimapZoomOut:Disable()
            else
...
function Pointer.MinimapButton_OnClick(self, button)
  if button == 'RightButton' then
    if ZGV.Pointer.ArrowFrame.waypoint == self.waypoint then
...
local dungeons = {
  [1] = {
    ...
    ['Maraudon'] = {
      ...
      l2 = 48,
      type = 'D',
      floors = { { map = 'maraudon', rooms = {} } },
    },
    ['Ragefire Chasm'] = {
      l1 = 15,
      l2 = 16,
      type = 'D',
      floors = { { map = 'ragefirechasm', rooms = {} } },
    },
    ['Razorfen Downs'] = {
      l1 = 34,
      l2 = 37,
      type = 'D',
      floors = { { map = 'razorfendowns', rooms = {} } },
    },
    ['Razorfen Kraul'] = {
      l1 = 24,
      l2 = 27,
      type = 'D',
      floors = { { map = 'razorfenkraul', rooms = {} } },
    },
    ['Wailing Caverns'] = {
      l1 = 17,
      l2 = 20,
      type = 'D',
      floors = { { map = 'wailingcaverns', rooms = {} } },
    },
    ["Zul'Farrak"] = {
      l1 = 43,
      l2 = 46,
      type = 'D',
      floors = { { map = 'zulfarrak', rooms = {} } },
    },
    ["Ahn'Qiraj"] = {
      l1 = 60,
      l2 = 63,
      type = 'R',
      floors = { { map = 'ahnqiraj', rooms = {} } },
    },
    ["Ruins of Ahn'Qiraj"] = {
      l1 = 60,
      l2 = 63,
      type = 'R',
      floors = { { map = 'ruinsofahnqiraj', rooms = {} } },
    },
    ["Onyxia's Lair"] = {
      l1 = 80,
      l2 = 83,
      type = 'R',
      floors = { { map = 'onyxiaslair', rooms = {} } },
    },
  },
  [2] = {
    ['Blackrock Depths'] = {
      ...
      l2 = 56,
      type = 'D',
      floors = { { map = 'blackrockdepths', rooms = {} } },
    },
    ['Blackrock Spire'] = {
      l1 = 57,
      l2 = 63,
      type = 'D',
      floors = { { map = 'blackrockspire', rooms = {} } },
    },
    ['Gnomeregan'] = {
      l1 = 25,
      l2 = 28,
      type = 'D',
      floors = { { map = 'gnomeregan', rooms = {} } },
    },
    ['Scarlet Monastery'] = {
      l1 = 32,
      l2 = 35,
      type = 'D',
      floors = { { map = 'scarletmonastery', rooms = {} } },
    },
    ['Scholomance'] = {
      l1 = 55,
      l2 = 65,
      type = 'D',
      floors = { { map = 'scholomance', rooms = {} } },
    },
    ['Shadowfang Keep'] = {
      l1 = 18,
      l2 = 21,
      type = 'D',
      floors = { { map = 'shadowfangkeep', rooms = {} } },
    },
    ['Stratholme'] = {
      l1 = 55,
      l2 = 65,
      type = 'D',
      floors = { { map = 'stratholme', rooms = {} } },
    },
    ['Sunken Temple'] = {
      l1 = 55,
      l2 = 65,
      type = 'D',
      floors = { { map = 'sunkentemple', rooms = {} } },
    },
    ['The Deadmines'] = {
      l1 = 17,
      l2 = 20,
      type = 'D',
      floors = { { map = 'deadmines', rooms = {} } },
    },
    ['The Stockade'] = {
      l1 = 22,
      l2 = 25,
      type = 'D',
      floors = { { map = 'stockade', rooms = {} } },
    },
    ['Uldaman'] = {
      l1 = 37,
      l2 = 40,
      type = 'D',
      floors = { { map = 'uldaman', rooms = {} } },
    },
    ['Blackwing Lair'] = {
      l1 = 60,
      l2 = 63,
      type = 'R',
      floors = { { map = 'blackwinglair', rooms = {} } },
    },
    ['Molten Core'] = {
      l1 = 60,
      l2 = 63,
      type = 'R',
      floors = { { map = 'moltencore', rooms = {} } },
    },
    ["Zul'Gurub"] = {
      l1 = 57,
      l2 = 63,
      type = 'R',
      floors = { { map = 'zulgurub', rooms = {} } },
    },
    ["Zul'Aman"] = {
      l1 = 70,
      l2 = 73,
      type = 'R',
      floors = { { map = 'zulaman', rooms = {} } },
    },
  },
  [3] = {
    ['Auchindoun: Auchenai Crypts'] = {
      ...
      l2 = 67,
      type = 'D',
      floors = { { map = 'auchenaicrypts', rooms = {} } },
    },
    ['Auchindoun: Mana-Tombs'] = {
      l1 = 64,
      l2 = 66,
      type = 'D',
      floors = { { map = 'manatombs', rooms = {} } },
    },
    ['Auchindoun: Sethekk Halls'] = {
      l1 = 67,
      l2 = 68,
      type = 'D',
      floors = { { map = 'sethekkhalls', rooms = {} } },
    },
    ['Auchindoun: Shadow Labyrinth'] = {
      l1 = 67,
      l2 = 75,
      type = 'D',
      floors = { { map = 'shadowlabyrinth', rooms = {} } },
    },
    ['Caverns of Time: Old Hillsbrad Foothills'] = {
      l1 = 66,
      l2 = 68,
      type = 'D',
      floors = { { map = 'oldhillsbrad', rooms = {} } },
    },
    ['Caverns of Time: The Black Morass'] = {
      l1 = 68,
      l2 = 75,
      type = 'D',
      floors = { { map = 'blackmorass', rooms = {} } },
    },
    ['Coilfang Reservoir: The Slave Pens'] = {
      l1 = 62,
      l2 = 64,
      type = 'D',
      floors = { { map = 'slavepens', rooms = {} } },
    },
    ['Coilfang Reservoir: The Steamvault'] = {
      l1 = 67,
      l2 = 75,
      type = 'D',
      floors = { { map = 'steamvault', rooms = {} } },
    },
    ['Coilfang Reservoir: The Underbog'] = {
      l1 = 63,
      l2 = 65,
      type = 'D',
      floors = { { map = 'underbog', rooms = {} } },
    },
    ['Hellfire Citadel: Hellfire Ramparts'] = {
      l1 = 59,
      l2 = 62,
      type = 'D',
      floors = { { map = 'hellfireramparts', rooms = {} } },
    },
    ['Hellfire Citadel: The Blood Furnace'] = {
      l1 = 61,
      l2 = 63,
      type = 'D',
      floors = { { map = 'bloodfurnace', rooms = {} } },
    },
    ['Hellfire Citadel: The Shattered Halls'] = {
      l1 = 67,
      l2 = 75,
      type = 'D',
      floors = { { map = 'shatteredhalls', rooms = {} } },
    },
    ["Magisters' Terrace"] = {
      l1 = 68,
      l2 = 75,
      type = 'D',
      floors = { { map = 'magistersterrace', rooms = {} } },
    },
    ['The Eye: The Arcatraz'] = {
      l1 = 68,
      l2 = 75,
      type = 'D',
      floors = { { map = 'arcatraz', rooms = {} } },
    },
    ['The Eye: The Botanica'] = {
      l1 = 67,
      l2 = 75,
      type = 'D',
      floors = { { map = 'botanica', rooms = {} } },
    },
    ['The Eye: The Mechanar'] = {
      l1 = 67,
      l2 = 75,
      type = 'D',
      floors = { { map = 'mechanar', rooms = {} } },
    },
    ...
    ['Black Temple'] = {
      ...
      l2 = 73,
      type = 'R',
      floors = { { map = 'blacktemple', rooms = {} } },
    },
    ['Coilfang Reservoir: Serpentshrine Cavern'] = {
      l1 = 70,
      l2 = 73,
      type = 'R',
      floors = { { map = 'serpentshrinecavern', rooms = {} } },
    },
    ["Hellfire Citadel: Magtheridon's Lair"] = {
      l1 = 70,
      l2 = 73,
      type = 'R',
      floors = { { map = 'magtheridonslair', rooms = {} } },
    },
    ['Karazhan'] = {
      l1 = 70,
      l2 = 73,
      type = 'R',
      floors = { { map = 'karazhan', rooms = {} } },
    },
    ['Sunwell Plateau'] = {
      l1 = 70,
      l2 = 73,
      type = 'R',
      floors = { { map = 'sunwellplateau', rooms = {} } },
    },
    ['Tempest Keep: Tempest Keep'] = {
      l1 = 70,
      l2 = 73,
      type = 'R',
      floors = { { map = 'tempestkeep', rooms = {} } },
    },
  },
  [4] = {
  ...
  if event == 'WORLD_MAP_UPDATE' then
    ...
    elseif IsInInstance() and GetPlayerMapPosition('player') == 0 then
      ...
      if inst then
        ...
        if room then
          --ZGV:Print("room")
          self.youarehere:SetPoint(
            'CENTER',
            self,
            'TOPLEFT',
            room.x * self:GetWidth(),
            -room.y * self:GetHeight()
          )
      ...
      for way, w in pairs(ZGV.Pointer.waypoints) do
        ...
        if way.worldmapFrame:IsShown() and way.OnEvent then
          way:OnEvent(event, ...)
        end
      end
    end
...
function Pointer.ArrowFrame_OnUpdate(self, elapsed)
  ...
  if profile.hidearrowwithguide and self.waypoint.type == 'way' and not ZGV.Frame:IsVisible() then
    self:Hide()
    return
  end
  ...
  if dist <= 10.0 then
    ...
    if profile.arrowcolordir then
      ...
      if not initialdist then
        initialdist = dist
      end
      if initialdist > 500 then
        initialdist = 500
      end
      if initialdist < 100 then
        initialdist = 100
      end
  ...
  if eta_elapsed >= 0.2 then
    ...
    if last_distance == dist then
      stoptime = stoptime + eta_elapsed
    else
      stoptime = 0
    end
    ...
    if ZGV.db.profile.audiocues and IsFlying() then
      local t = GetTime()
      if lastplayerangle ~= playerangle then
        lastturntime = t
      end
      if last_distance == dist then
        laststoptime = t
  ...
  self.title:SetText(
    (title and '|cffffffff' .. title .. '|r\n' or '')
      .. (disttxt and '|cffffcc00' .. disttxt .. '|r' or '')
      .. (etatxt and '  |cffff7700' .. etatxt .. '|r' or '')
  )
end
...
function Pointer.Overlay_OnUpdate(frame, but, ...)
  ...
  if IsMouseButtonDown('LeftButton') and IsShiftKeyDown() then
    ...
    if leftbutdown then
      ...
      --ZGV:Print(x.." "..y)
      x = (x - (frame:GetLeft() * frame:GetEffectiveScale()))
        / (frame:GetWidth() * frame:GetEffectiveScale())
      y = (y - (frame:GetBottom() * frame:GetEffectiveScale()))
...
function Pointer:SetCorpseArrow()
  if self.corpsearrow then
    return
  end
  if not UnitIsDeadOrGhost('player') then
    ZGV:Debug('Pointer.SetCorpseArrow: not dead!')
    return
  end
  ...
  if x > 0 and y > 0 and c > 0 and z > 0 then
    ...
    self:SetWaypoint(
      ...
      x,
      y,
      { title = L['pointer_corpselabel' .. math.random(5)], type = 'corpse' }
    )
