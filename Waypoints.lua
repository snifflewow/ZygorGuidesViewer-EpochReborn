...
me.WaypointFunctions['tomtom'] = {
  ...
  disconnect = function(self)
    ...
    if StaticPopupDialogs['TOMTOM_REMOVE_ALL_CONFIRM'] then
...
me.WaypointFunctions['cart2'] = {
  isready = function()
    return not not (
      Cartographer_Notes
      and Cartographer_Notes:IsActive()
      and Cartographer_Notes.externalDBs
    )
  end,

  clearmapnotes = function(self) end,
  addmapnote = function(self, zone, x, y, data) end,
...
me.WaypointFunctions['internal'] = {
  ...
  setwaypoint = function(self, goalnumORx, y, title)
    if UnitIsDeadOrGhost('player') then
      return
    end -- don't overwrite the stinking arrow
    ...
    if not y then
      local goals = {}
      local firstpoint
      if not self.CurrentStep or not self.CurrentStep.goals then
        return
      end
      if goalnumORx then
      ...
      self.Pointer:SetWaypoint(
        nil,
        nil,
        goalnumORx,
        y,
        { title = title, persistent = true, overworld = true }
      )
    end
  end,
  addmapnote = function(self, zone, x, y, data)
...
for k, v in pairs(me.WaypointFunctions) do
  setmetatable(v, nilfuncs)
end

function me:SetWaypoint(...)
  if not self.ConnectedWaypointer then
    return
  end
  if ... ~= false and self.db.profile.hidearrowwithguide and not ZGV.Frame:IsShown() then
    return
  end
...
function me:UnsetWaypointAddon()
  ...
  if not addon or addon == 'none' then
    return
  end
...
function me:IsWaypointAddonReady(addon)
  if not addon then
    addon = self.db.profile.waypointaddon
  end
...
function me:UpdateCartographerExport()
  if (self.db.profile.waypointaddon ~= 'cart2') and (self.db.profile.waypointaddon ~= 'cart3') then
    return
  end -- or (not self.iconsregistered)
...
function me:CreateTomTomWaypoints(goalnum)
  ...
  for i = #self.CurrentStep.goals, 1, -1 do
    ...
    if goal.x and not goal.force_noway then
      ...
      if way then
        table.insert(self.TomTomWaypoints, way)
      end
    end
  end
end
...
function me:GetMapZoneNumbers(zonename)
  if zonename == self.BZL['Plaguelands: The Scarlet Enclave'] then
    return 5, 1
  end
