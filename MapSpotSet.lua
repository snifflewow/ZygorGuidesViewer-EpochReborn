        ...
        for itemstr in params:gmatch('%s*(.-)%s*,') do
          ...
          if alias then
            ...
            if not object.item.id then --and not baditems[name]
              ZGV:Print(
                '|cffff6600ERROR: unknown item |cffffaa00'
                  .. name
                  .. '|cffff6600 in the gold guide|r'
              )
              --baditems[name]=true
            end
          end
          ...
          if object.perc then
            object.perc = tonumber(object.perc)
          end
          ...
          if spot.itemsource == 'vendor' then
            object.vendor = 1
          end
        end
      end
...
function MapSpotSet:GetSpotsInRange(into)
  ...
  for s, spot in ipairs(self.spots) do
    if
      spot.waypoint
      and spot.waypoint.minimapFrame
      and spot.waypoint.minimapFrame:IsShown()
      and spot.waypoint.minimapFrame.dist
      and spot.waypoint.minimapFrame.dist <= ZGV.db.profile.golddetectiondist
    then
      tinsert(ret, spot)
    end
  end
  table.sort(ret, function(a, b)
    return a.waypoint.minimapFrame.dist
        and a.waypoint.minimapFrame.dist
        and a.waypoint.minimapFrame.dist < b.waypoint.minimapFrame.dist
      or false
  end)
  return ret
end
