...
-- Skin data now lives in Skins/Styles/Style-*.lua and is applied by
-- ZGV.Skins:Apply(). See Skins/Skins.lua.
...
function me:OnInitialize()
  ...
  -- Skins: styles are registered from Skins/Styles/Style-*.lua.
  -- LoadSkin is kept as a thin wrapper so older call sites keep working.
  function LoadSkin(name)
    ZGV.Skins:Apply(name)
  end
  ...
  if ZygorTalentAdvisor and ZygorTalentAdvisor.revision > self.revision then
    ...
    for i = 1, guidebuttoncount do
      ...
      local btn = CreateFrame('Button', name, ZygorGuidesViewerFrame, 'SecureActionButtonTemplate')
      btn:SetSize(60, 60)
      btn:SetHitRectInsets(12, 12, 12, 12)

      local spacing = -10
      local xOffset = 4 + (i - 1) * (60 + spacing)
      btn:SetPoint('BOTTOMLEFT', ZygorGuidesViewerFrame, 'TOPLEFT', xOffset, 2)
      ...
      icon:SetPoint('TOPLEFT', btn, 'TOPLEFT', 14, -14)
      icon:SetPoint('BOTTOMRIGHT', btn, 'BOTTOMRIGHT', -14, 14)
...
function me:SetMacroButton(slot, text, icon)
...
function me:SetNextMacroButton(text, icon)
  me:SetMacroButton(macroIndex, text, icon)
...
function me:OnFirstQuestLogUpdate()
  ...
  self:UpdateSkin() -- re-apply the style now the frame is laid out
  self:AlignFrame()
...
function me:FocusStep(num, quiet)
  ...
  -- Bumped only on genuine step focus. Used by path goals to tell a real
  -- step change from PrepareCompletion being re-run (e.g. leaving combat).
  self.stepserial = (self.stepserial or 0) + 1
...
function me:AlignFrame()
  ...
  ZygorGuidesViewerFrame_Border_Top:SetPoint(
    ...
    self.db.profile.Boarder_topright_offsetY * UP
  )
  local tx = ZygorGuidesViewerFrame_Border_Top:GetTexture()
  ZygorGuidesViewerFrame_Border_Top:SetTexture(1)
  local tiletop = self.db.profile.Border_Top_Tile
  if tiletop == nil then
    tiletop = true
  end
  ZygorGuidesViewerFrame_Border_Top:SetTexture(tx, tiletop)
  ZygorGuidesViewerFrame_Border_Top:SetTexCoord(UPcoords(0, 1, 0, 1))

  -- Position Left Border using values from the active style
  ...
  -- Position Right Border using values from the active style
...
function me:UpdateSkin()
  -- Texture folder comes from the style, so a style can point at a shared
  -- folder (texturedir) instead of one named after its own id.
  local style = ZGV.Skins:GetCurrentStyle()
  local texdir = (style and rawget(style, 'texturedir')) or self.db.profile.skin
  SKINDIR = DIR .. '\\Skin\\' .. texdir
  ZGV.SKINDIR = SKINDIR -- style OnApply handlers need this
  ...
  -- Apply the selected style's data (layout offsets, backdrop, colours).
  ZGV.Skins:Apply(self.db.profile.skin)
  self:UpdateLocking()
  self:AlignFrame()
end
...
local function FindGroup(self, rawpath, title)
  ...
  if s and not rawpath:match('^Epoch') then
