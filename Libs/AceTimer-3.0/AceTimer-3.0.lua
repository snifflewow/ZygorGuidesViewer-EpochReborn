  ...
  if type(callback) ~= 'string' and type(callback) ~= 'function' then
    ...
    error(
      MAJOR
        .. ': '
        .. error_origin
        .. "(callback, delay, arg): 'callback' - function or method name expected.",
      3
    )
  end
  if type(callback) == 'string' then
    if type(self) ~= 'table' then
      local error_origin = repeating and 'ScheduleRepeatingTimer' or 'ScheduleTimer'
      error(
        MAJOR .. ': ' .. error_origin .. '("methodName", delay, arg): \'self\' - must be a table.',
        3
      )
    end
    if type(self[callback]) ~= 'function' then
      ...
      error(
        MAJOR
          .. ': '
          .. error_origin
          .. '("methodName", delay, arg): \'methodName\' - method not found on target object.',
        3
      )
    end
  end
...
function AceTimer:CancelTimer(handle, silent)
  if not handle then
    return
  end -- nil handle -> bail out without erroring
    ...
    if not timer.callback then
      geterrorhandler()(
        MAJOR
          .. ": CancelTimer(handle[, silent]): '"
          .. tostring(handle)
          .. "' - timer already cancelled or expired"
      )
      return false
    end
    timer.callback = nil -- don't run it again
...
function AceTimer:TimeLeft(handle)
  ...
  if not timer then
    geterrorhandler()(
      MAJOR .. ": TimeLeft(handle): '" .. tostring(handle) .. "' - no such timer registered"
    )
    return false
  end
...
local function OnEvent(this, event)
  ...
  if n > BUCKETS then
    DEFAULT_CHAT_FRAME:AddMessage(
      MAJOR
        .. ": Warning: The addon/module '"
        .. tostring(self)
        .. "' has "
        .. n
        .. " live timers. Surely that's not intended?"
    )
  end
