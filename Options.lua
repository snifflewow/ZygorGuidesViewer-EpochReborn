...
-- Defaults come from the shared 'base' style rather than a hardcoded table.
local PROFILE_DEFAULTS =
  MergeTables(BASE_PROFILE_DEFAULTS, ZGV.Skins:GetStyleValueSet('base'))
...
function me:Options_DefineOptions()
    ...
    args = {
      ...
      debug = {
        ...
        set = function()
          ...
          ZGV:Print(
            'Debugging: ' .. (self.db.profile.debug and '|cff00ff88ON|r' or '|cffff0055OFF|r')
          )
        end,
        order = -10,
      },
      ...
      window = {
        ...
        args = {
          ...
          skin = {
            ...
            type = 'select',
            -- Built from whatever styles are registered, so adding a style
            -- needs no edit here.
            values = function()
              return ZGV.Skins:GetStyleValues()
            end,
            set = function(_, n)
              self.db.profile.skin = n
              -- Apply() copies the style's data (including skincolors) into the profile.
              ZGV.Skins:Apply(n)
              self:UpdateSkin()
              self:AlignFrame()
              self:UpdateLocking()
          ...
          resizeup = {
            ...
            set = function(i, v)
              ...
              self:UpdateSkin() -- re-run the style's OnApply for the new orientation
              self:AlignFrame()
              -- THIS SUCKS.
            end,
            order = 4.2,
  ...
  self.optionsprogress = {
    ...
    args = {
      ...
      desc1 = {
        type = 'header',
        name = L['opt_progressbackcolor_desc'],
        order = 11,
      },
      goalbackaux = {
...
function me:Options_SetupConfig()
  ...
  LibStub('AceConfig-3.0'):RegisterOptionsTable(
    'ZygorGuidesViewer-Gold',
    self.optionsgold,
    'zggold'
  )
  LibStub('AceConfig-3.0'):RegisterOptionsTable(
    'ZygorGuidesViewer-Conv',
    self.optionsconv,
    'zgconv'
  )
  LibStub('AceConfig-3.0'):RegisterOptionsTable(
...
function me:Options_SetupBlizConfig()
  ...
  LibStub('AceConfigDialog-3.0'):AddToBlizOptions(
    'ZygorGuidesViewer-Map',
    self.optionsmap.name,
    self.options.name
  )
  LibStub('AceConfigDialog-3.0'):AddToBlizOptions(
    'ZygorGuidesViewer-Gold',
    self.optionsgold.name,
    self.options.name
  )
  LibStub('AceConfigDialog-3.0'):AddToBlizOptions(
    'ZygorGuidesViewer-Conv',
    self.optionsconv.name,
    self.options.name
  )
